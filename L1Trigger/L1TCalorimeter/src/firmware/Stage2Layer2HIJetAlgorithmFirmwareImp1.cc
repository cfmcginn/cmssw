...
void l1t::Stage2Layer2HIJetAlgorithmFirmwareImp1::create(const std::vector<l1t::CaloTower> & towers,
  ...
  for(unsigned int i = 0; i < towers.size(); ++i){
    tempTowMax[towers.at(i).hwEta()] += 1;
  }

  for(std::map<Int_t,Int_t>::iterator it=towMax.begin(); it!=towMax.end(); ++it){
    if(it->second < tempTowMax[it->first]) towMax[it->first] = tempTowMax[it->first];
  }


  if(PUSubMethod == "PhiRingPP" || PUSubMethod == "PhiRingPPExclude" || PUSubMethod == "PhiRingPPTower" || PUSubMethod == "PhiRingPPTowerMask" || PUSubMethod == "PhiRingHITower" || PUSubMethod == "PhiRingHIRegion"){
    phiRingPUEstimate(&etaPUEstimate, &etaPUN, towers, PUSubMethod);
	...
	if(i == 29) continue;
	if(i == -29) continue;
	etaPUEstimate[i] = (int)etaPUEstimate[i]/(int)etaPUN[i];
      }
    }

  }
  else if(PUSubMethod == "PhiRingPPTowerMedian"){
    phiRingMedianPUEstimate(&etaPUEstimate, towers, PUSubMethod);
  }
  ...
  for (int etaSide=1; etaSide>=-1; etaSide-=2) {
	...
	for ( int iphi=1; iphi<=CaloTools::kHBHENrPhi; ++iphi ) {
	  ...
	  int seedEt = tow.hwPt();
	  int iEt = seedEt;
	  if(true/*etaPUN[ieta] > 0*/){
	    if(PUSubMethod == "PhiRingPPTower" || PUSubMethod == "PhiRingPPTowerMask"|| PUSubMethod == "PhiRingHITower" || PUSubMethod == "PhiRingHIRegion" || PUSubMethod == "PhiRingPPTowerMedian"){
	      iEt -= etaPUEstimate[ieta];
	      seedEt -= etaPUEstimate[ieta];
	    }
	  }
	  ...
	  for( int deta = -4; deta < 5; ++deta ) {
		...
		if(ietaTest == 27 || ietaTest == -27) continue;
	      ...
	      */
	      ...
	      if(PUSubMethod == "PhiRingPPTower" || PUSubMethod == "PhiRingPPTowerMask" || PUSubMethod == "PhiRingHITower" || PUSubMethod == "PhiRingHIRegion" || PUSubMethod == "PhiRingPPTowerMedian"){
		if(true/*etaPUN[ietaTest] > 0*/) towEt -= etaPUEstimate[ietaTest];
		if(towEt < 0) towEt = 0;
	      }
...
void l1t::Stage2Layer2HIJetAlgorithmFirmwareImp1::accuSort(std::vector<l1t::Jet> & jets){
  ...
  */
  ...
  if(accumEtaNegBar.at(6).hwPt()==accumEtaNegBar.at(5).hwPt() && accumEtaNegBar.at(6).hwEta()==accumEtaNegBar.at(5).hwEta()
     && accumEtaNegBar.at(6).hwPhi() > accumEtaNegBar.at(5).hwPhi()){
    accumEtaNegBar.at(5)=accumEtaNegBar.at(6);
  }
  ...
  */
  ...
  std::cout << "COMPLETED JET PUT" << std::endl;
}
...
void l1t::Stage2Layer2HIJetAlgorithmFirmwareImp1::phiRingMedianPUEstimate(std::map<int, int>* etaVal, const std::vector<l1t::CaloTower> & towers, std::string algoStr)
{
  ...
  for(unsigned int i = 0; i < towers.size(); ++i){
    vectMap[towers.at(i).hwEta()].push_back(towers.at(i).hwPt());
  }

  for(int i = -41; i <= 41; ++i){
    if(i == 0) continue;
    else if(i == -29) continue;
    else if(i == 29) continue;

    std::vector<int> temp = vectMap[i];
    std::sort(std::begin(temp), std::end(temp));
    (*etaVal)[i] = temp.at(temp.size()/2);
  }
